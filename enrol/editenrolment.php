...
/**
 * User enrollment edit script.
 *
 * This page allows the current user to edit a user enrollment.
...
require('../config.php');
require_once("$CFG->dirroot/enroll/locallib.php"); // Required for the course enrollment manager.
require_once("$CFG->dirroot/enroll/renderer.php"); // Required for the course enrollment users table.
require_once("$CFG->dirroot/enroll/editenrolment_form.php"); // Forms for this page.
...
require_capability('enroll/'.$instance->enroll.':manage', context_course::instance($course->id, MUST_EXIST));

// Get the enrollment manager for this course.
$manager = new course_enrolment_manager($PAGE, $course, $filter);
// Get an enrollment users table object. Doing this will automatically retrieve the the URL params
...
$url = new moodle_url('/enroll/editenrollment.php', $returnurl->params());

$PAGE->set_url($url);
$PAGE->set_pagelayout('admin');
navigation_node::override_active_url($usersurl);

// Get the enrollment edit form.
...
if ($mform->is_cancelled()) {
    ...
    if ($manager->edit_enrollment($ue, $data)) {
        redirect($returnurl);
    }
}

$fullname = fullname($user);
$title = get_string('editenrollment', 'core_enroll');
