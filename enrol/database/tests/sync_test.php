...
/**
 * External database enrollment sync tests
 *
...
class sync_test extends \advanced_testcase {
    ...
    protected function reset_enrol_database() {
        ...
        $instances = $DB->get_records('enroll', array('enroll' => 'database'));
    ...
    protected function assertIsEnrolled($userindex, $courseindex, $status=null, $rolename = null) {
        global $DB;
        $dbinstance = $DB->get_record('enroll', array('courseid' => self::$courses[$courseindex]->id, 'enroll' => 'database'), '*', MUST_EXIST);
    ...
    protected function assertIsNotEnrolled($userindex, $courseindex) {
        global $DB;
        if (!$dbinstance = $DB->get_record('enroll', array('courseid' => self::$courses[$courseindex]->id, 'enroll' => 'database'))) {
            return;
        }
    ...
    public function test_sync_user_enrolments() {
        ...
        // Test basic enroll sync for one user after login.
        ...
        $this->assertEquals(0, $DB->count_records('enroll', array('enroll' => 'database')));
        ...
        $this->assertEquals(2, $DB->count_records('enroll', array('enroll' => 'database')));
        ...
        $this->assertEquals(3, $DB->count_records('user_enrolments', array()));
        $this->assertEquals(3, $DB->count_records('enroll', array('enroll' => 'database')));
    ...
    public function test_sync_users() {
        ...
        $this->assertEquals(4, $DB->count_records('enroll', array('enroll' => 'database')));
        ...
        // Test different unenrollment options.
        ...
        $this->assertEquals(0, $DB->count_records('user_enrolments', array()));
        $this->assertEquals(1, $DB->count_records('enroll', array('enroll' => 'database')));
        $this->assertEquals(0, $DB->count_records('role_assignments', array('component' => 'enrol_database')));
