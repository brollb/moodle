...
/**
 * This page shows all course enrollment options for current user.
 *
 * @package    core_enroll
...
if (!isloggedin()) {
    ...
    if (empty($referer)) {
        ...
        // they should be redirected to the course page they are trying to enroll on after logging in.
...
$PAGE->set_url('/enroll/index.php', array('id'=>$course->id));
...
if (\core\session\manager::is_loggedinas() and $USER->loginascontext->contextlevel == CONTEXT_COURSE) {
    throw new \moodle_exception('loginasnoenroll', '', $CFG->wwwroot.'/course/view.php?id='.$USER->loginascontext->instanceid);
}
...
// get all enroll forms available in this course
...
foreach($enrolinstances as $instance) {
    ...
    $form = $enrols[$instance->enroll]->enrol_page_hook($instance);
    if ($form) {
...
$PAGE->navbar->add(get_string('enrolmentoptions','enroll'));

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('enrolmentoptions','enroll'));
...
if (!$forms) {
    if (isguestuser()) {
        notice(get_string('noguestaccess', 'enroll'), get_login_url());
    } else if ($returnurl) {
        notice(get_string('notenrollable', 'enroll'), $returnurl);
    } else {
        $url = get_local_referer(false);
        if (empty($url)) {
            $url = new moodle_url('/index.php');
        }
        notice(get_string('notenrollable', 'enroll'), $url);
    }
}
