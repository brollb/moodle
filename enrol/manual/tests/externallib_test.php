...
require_once($CFG->dirroot . '/enroll/manual/externallib.php');
...
class externallib_test extends externallib_advanced_testcase {
    ...
    public function test_enrol_users() {
        ...
        $roleid = $this->assignUserCapability('enroll/manual:enroll', $context1->id);
        ...
        $this->assignUserCapability('enroll/manual:enroll', $context2->id, $roleid);
        ...
        $this->unassignUserCapability('enroll/manual:enroll', $context1->id, $roleid);
        try {
            enrol_manual_external::enrol_users(array(
                array('roleid' => 3, 'userid' => $user1->id, 'courseid' => $course1->id),
            ));
            $this->fail('Exception expected if not having capability to enroll');
        } catch (\moodle_exception $e) {
            $this->assertInstanceOf('required_capability_exception', $e);
            $this->assertSame('nopermissions', $e->errorcode);
        }
        $this->assignUserCapability('enroll/manual:enroll', $context1->id, $roleid);
        ...
        $DB->delete_records('user_enrolments');
        $DB->delete_records('enroll', array('courseid' => $course2->id));
        try {
            enrol_manual_external::enrol_users(array(
                array('roleid' => 3, 'userid' => $user1->id, 'courseid' => $course1->id),
    ...
    public function test_unenrol_user_single() {
        ...
        $enrolinstance = $DB->get_record('enroll', array('courseid' => $course->id, 'enroll' => 'manual'), '*', MUST_EXIST);
        // Set the capability for the user.
        $roleid = $this->assignUserCapability('enroll/manual:enroll', $coursecontext);
        $this->assignUserCapability('enroll/manual:unenroll', $coursecontext, $roleid);
        ...
        // Create a student and enroll them into the course.
        $student = $this->getDataGenerator()->create_user();
        $enroll->enrol_user($enrolinstance, $student->id);
        $this->assertTrue(is_enrolled($coursecontext, $student));
        // Call the web service to unenroll.
    ...
    public function test_unenrol_user_multiple() {
        ...
        $enroll->enrol_user($enrolinstance, $student1->id);
        $this->assertTrue(is_enrolled($coursecontext, $student1));
        $student2 = $this->getDataGenerator()->create_user();
        $enroll->enrol_user($enrolinstance, $student2->id);
    ...
    /**
     * Test for unenroll if user does not exist.
     * @throws coding_exception
     */
    public function test_unenrol_user_error_not_exist() {
        ...
        $DB->delete_records('enroll', array('id' => $enrolinstance->id));
        try {
            enrol_manual_external::unenrol_users(array(
                array('userid' => $student->id + 1, 'courseid' => $course->id),
            ));
