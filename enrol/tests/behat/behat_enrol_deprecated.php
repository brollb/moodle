...
class behat_enrol_deprecated extends behat_deprecated_base {
    ...
    /**
     * Adds the specified enrollment method to the current course filling the form with the provided data.
     *
     * @Given /^I add "(?P<enrolment_method_name_string>(?:[^"]|\\")*)" enrollment method with:$/
     * @param string $enrolmethod
     * @param TableNode $table
     *
     * @deprecated since 4.0 MDL-72090. We now need the course to enroll in. Please use i_add_enrolment_method_for_with()
     * @todo MDL-71733 This will be deleted in Moodle 4.4.
     */
    public function i_add_enrolment_method_with($enrolmethod, TableNode $table) {
        $this->deprecated_message(['behat_enroll::i_add_enrolment_method_for_with']);

        // Navigate to enrollment method page.
